...
export type Validate = (value: any) => boolean | Promise<boolean>;
...
function createValidation(validate: (value: any) => boolean | Promise<boolean>, error: ErrorInfo): Rule {
    return {
        validate: validate,
        error: error
    }
...
export let rules = {
    ...
    email(error?: ErrorInfo): Rule {
        var validate = (value: any) => emailRegex.test(value);
        return createValidation(validate, error || msgs.required);
    },
    /**
     * 验证字段最小长度
    ...
    minLength(length: Value<number>, error?: ErrorInfo): Rule {
        var validate = (value: any) => (value || '').length >= calc(length);
    ...
    greaterThan(value: Value<number | Date>, error?: ErrorInfo) {
        var validate = (o: any) => elementValueCompare(o, calc(value)) == 'greaterThan';
    ...
    lessThan(value: Value<number | Date | string>, error?: ErrorInfo) {
        var validate = (o: any) => elementValueCompare(o, calc(value)) == 'lessThan';
    ...
    equal(value: Value<number | Date | string>, error?: ErrorInfo) {
        var validate = (o: any) => elementValueCompare(o, calc(value)) == 'equal';
    ...
    ip(error?: ErrorInfo): Rule {
        var validate = (value: any) => ipRegex.test(value);
    ...
    url(error?: ErrorInfo): Rule {
        var validate = (value: any) => urlRegex.test(value);
    ...
    mobile(error?: ErrorInfo): Rule {
        var validate = (value: any) => mobileRegex.test(value);
    ...
    numeric(error?: ErrorInfo): Rule {
        var validate = (value: string) => numericRegex.test(value);
     ...
     * @param error 错误提示文字
     */
    custom(validate: (value: any) => boolean | Promise<boolean>, error?: ErrorInfo) {
...
function elementValueCompare<T extends number | Date | string>(value: string, otherValue: T): 'lessThan' | 'greaterThan' | 'equal' {
    ...
    let elementValue: number | Date | string | null;
